...
| **關鍵規則** | 問題要聚焦（不要一次問太多，但相關的追問可以自然合併）；區分「題目」vs「問題」；方法要匹配問題 |
...
| **做什麼** | 撰寫具備嚴謹論證結構的學術文章 |
| **角色** | 學術寫作專家，**先骨架、再寫作** |
| **進入方式** | 從零開始 → 完整流程；已有草稿 → 跳到精修；展開大綱 → 從筆記建骨架 |
...
| **角色狀態標籤** | 每則回覆開頭標示角色（`[Athena — 審稿]`、`[Calliope — 修訂]` 等），讓使用者隨時知道目前是哪個角色 |
...
| `examples.md` | 給人看的實際案例示範（案例一～五，展示各 skill 的完整對話流程——skill 不會載入此檔，僅供使用者參考） |
